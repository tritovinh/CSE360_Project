...
public class ModelDiscussion {
	...
	/** Approves a reply by id; returns empty string on success. */
	public static String approveReply(int replyId) {
		boolean ok = theDatabase.approveReply(replyId);
		return ok ? "" : "Unable to approve reply.";
	}

	public static String disapproveReply(int replyId) {
		boolean ok = theDatabase.disapproveReply(replyId);
		return ok ? "" : "Unable to disapprove reply.";
	}
}
